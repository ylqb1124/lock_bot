...
集群视图当前的计算资源口径为 **56 个计算节点**，按每节点 8 张 XPU 计算，视图总资源为 **448 张卡**。Lock Bot 可能额外返回 bdc 节点，但它们没有 Monquery 数据，不参与节点使用率或锁定趋势。
...
共享范围配置当前包含 56 个计算节点。前端按该范围初始化 56 个节点，因此 Lock Bot 响应缺少节点时仍保留在资源分母中；bdc 节点不参与计算资源统计。待启用节点保存在 `pendingNodeGroups`，仅用于保留节点 ID、主机名和 IP；只有提升到 `nodeIds` 与带 `effectiveFrom` 的 `nodeGroups` 后才会纳入统计。

### 2.2 当前 Monquery 查询清单的实现边界

`api.js` 的当前负载查询使用 56 个 `node*` namespace，不包含 `bdc`。`trend-service.cjs` 对历史 XPU/显存趋势按 `nodeGroups.effectiveFrom` 切分请求窗口，只查询每个采样点已经生效的计算节点；两者均不决定 Lock Bot 资源总数。

这意味着当前实现存在两个不能混用的分母：

- **资源总览**：当前按 56 节点、448 卡统计。
- **锁定趋势**：按每个采样点已生效的节点集合计算分子和分母；当前为 56 节点、448 卡。
- **Monquery 趋势查询**：按每个采样点已生效的计算 namespace 聚合 XPU 与显存，不包含 bdc；当前为 56 个节点。

页面和文档不得把 Monquery 的 56 节点查询清单写成集群视图的总节点数。

其中 `node32`、`node34`、`node35`、`node37` 至 `node51`、`node60` 至 `node69` 使用 `wxtky02-p800-8nic-vd` namespace，其他当前查询节点使用 `wxtky02-p800-backup-8nic-vd` namespace。
...
页面将所有成功响应的 Lock Bot `state` 规范化为当前的 56 个计算节点，并按卡合并，避免重复统计同一张卡。`bdc` 节点不会进入该计算资源集合。
...
“节点使用率”卡片的实际含义是当前锁定卡占 56 个计算节点总卡数的比例：

```text
节点使用率 = LOCKED 卡数 / (56 × 8) × 100%
```
...
bdc 锁定记录会在趋势计算时过滤；未在该采样时刻生效的计算节点也不会进入锁定率分子或分母。当前“节点使用率趋势”与顶部“节点使用率”均按 56 节点、448 卡计算。
...
- [ ] Lock Bot 返回 56 个计算节点和 bdc 节点时，顶部总节点显示 56、总卡数显示 448。
