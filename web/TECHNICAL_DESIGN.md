...
集群视图当前的计算资源口径为 **66 个计算节点**，按每节点 8 张 XPU 计算，视图总资源为 **528 张卡**。Lock Bot 可能额外返回 bdc 节点，但它们没有 Monquery 数据，不参与节点使用率或锁定趋势。
...
共享范围配置当前包含 66 个计算节点。前端按该范围初始化 66 个节点，因此 Lock Bot 响应缺少节点时仍保留在资源分母中；bdc 节点不参与计算资源统计。节点上线后会提升到 `nodeIds` 与带 `effectiveFrom` 的 `nodeGroups`；日期值在中国时间零点生效，精确时间必须带 `+08:00`。

### 2.2 当前 Monquery 查询清单的实现边界

`api.js` 的当前负载查询使用 66 个 `node*` namespace，不包含 `bdc`。`trend-service.cjs` 对历史 XPU/显存趋势按 `nodeGroups.effectiveFrom` 切分请求窗口，只查询每个采样点已经生效的计算节点；两者均不决定 Lock Bot 资源总数。

当前范围为 66 节点，但历史趋势必须按采样时间使用对应的节点分母：

- **资源总览**：当前按 66 节点、528 卡统计。
- **锁定趋势**：按每个采样点已生效的节点集合计算分子和分母；当前为 66 节点、528 卡。
- **Monquery 趋势查询**：按每个采样点已生效的计算 namespace 聚合 XPU 与显存，不包含 bdc；当前为 66 个节点。

页面和文档不得用当前的 66 节点范围重算 10:00 之前的历史趋势。

其中 `node32`、`node34`、`node35`、`node37` 至 `node51`、`node60` 至 `node79` 使用 `wxtky02-p800-8nic-vd` namespace，其他当前查询节点使用 `wxtky02-p800-backup-8nic-vd` namespace。
...
页面将所有成功响应的 Lock Bot `state` 规范化为当前的 66 个计算节点，并按卡合并，避免重复统计同一张卡。`bdc` 节点不会进入该计算资源集合。
...
当卡级数据尚不可用但整机 XPU 可用时，节点以整机 XPU `>= 10%` 判断。

“节点使用率”卡片的实际含义是当前锁定卡占 66 个计算节点总卡数的比例：

```text
节点使用率 = LOCKED 卡数 / (66 × 8) × 100%
```
...
bdc 锁定记录会在趋势计算时过滤；未在该采样时刻生效的计算节点也不会进入锁定率分子或分母。当前“节点使用率趋势”与顶部“节点使用率”均按 66 节点、528 卡计算。
...
- [ ] Lock Bot 返回 66 个计算节点和 bdc 节点时，顶部总节点显示 66、总卡数显示 528。
