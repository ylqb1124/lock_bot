...
集群视图的计算资源口径为 **46 个计算节点**，按每节点 8 张 XPU 计算，视图总资源为 **368 张卡**。Lock Bot 可能额外返回 bdc 节点，但它们没有 Monquery 数据，不参与节点使用率或锁定趋势。
...
共享范围配置固定了 46 个计算节点。前端按该范围初始化 46 个节点，因此 Lock Bot 响应缺少节点时仍保留在资源分母中；bdc 节点不参与计算资源统计。

### 2.2 当前 Monquery 查询清单的实现边界

`api.js` 和 `trend-service.cjs` 使用独立的 Monquery 节点清单：46 个 `node*` namespace，不包含 `bdc`。该清单仅服务于 XPU/显存当前负载查询及趋势聚合，不决定 Lock Bot 资源总数。

这意味着当前实现存在两个不能混用的分母：

- **资源总览和锁定趋势**：固定按 46 节点、368 卡统计。
- **Monquery 趋势查询**：固定查询 46 个计算 namespace，并不包含 bdc。

页面和文档不得把 Monquery 的 46 节点查询清单写成集群视图的总节点数。
...
页面将所有成功响应的 Lock Bot `state` 规范化为固定的 46 个计算节点，并按卡合并，避免重复统计同一张卡。`bdc` 节点不会进入该计算资源集合。
...
“节点使用率”卡片的实际含义是当前锁定卡占 46 个计算节点总卡数的比例：

```text
节点使用率 = LOCKED 卡数 / (46 × 8) × 100%
```
...
```text
LockRate(t) = uniqueLockedCards(t) / (46 × 8) × 100%
```

去重键为 `(规范化节点名, 卡号)`。有有效卡号的 `DEVICE` 记录只计一张卡；没有有效卡号的记录及节点级记录按 8 张卡展开。

bdc 锁定记录会在趋势计算时过滤，因此“节点使用率趋势”与顶部“节点使用率”使用同一套 46 节点、368 卡的计算资源口径。
...
- [ ] Lock Bot 返回 46 个计算节点和 bdc 节点时，顶部总节点显示 46、总卡数显示 368。
