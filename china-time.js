...
export function chinaDateRange(start, end) {
  const dates = [];
  const toDayStart = value => {
    const [year, month, day] = formatChinaDate(value).split('-').map(Number);
    return Date.UTC(year, month - 1, day) - CST_OFFSET_MS;
  };
  let timestamp = toDayStart(start);
  const last = toDayStart(end);
  for (; timestamp <= last; timestamp += DAY_MS) dates.push(formatChinaDate(timestamp));
  return dates;
}
